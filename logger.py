...
class Logger:
    ...
    def save_cpk(self, emergent=False):
        ...
        if not (os.path.exists(cpk_path) and emergent):
            fluid.save_dygraph(cpk, cpk_path)
...
class Visualizer:
    ...
    def visualize(self, driving, source, out):
        ...
        if 'occlusion_map' in out:
            ...
            occlusion_map = fluid.layers.interpolate(occlusion_map, out_shape=source.shape[1:3]).numpy()
        ...
        if 'sparse_deformed' in out:
            ...
            for i in range(out['sparse_deformed'].shape[1]):
                ...
                image = fluid.layers.interpolate(image, out_shape=source.shape[1:3])
                # image = F.interpolate(image, size=source.shape[1:3])
                mask = out['mask'][:, i:(i+1)].data.cpu().repeat(1, 3, 1, 1)
                mask = fluid.layers.interpolate(mask, out_shape=source.shape[1:3])
