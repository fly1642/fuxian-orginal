...
from frames_dataset import PairedDataset, FramesDataset
...
def normalize_kp(kp_source, kp_driving, kp_driving_initial, adapt_movement_scale=False,
    ...
    if use_relative_movement:
        ...
        if use_relative_jacobian:
            jacobian_diff = fluid.layers.matmul(kp_driving['jacobian'], fluid.dygraph.to_variable(np.linalg.inv(kp_driving_initial['jacobian'].numpy())))
            kp_new['jacobian'] = fluid.layers.matmul(jacobian_diff, kp_source['jacobian'])
...
def animate(config, generator, kp_detector, checkpoint, log_dir, dataset):
    ...
    dataloader = fluid.io.batch(dataset.reader, 1, drop_last=True)
    ...
    # for it, x in tqdm(enumerate(dataloader)):
        # with torch.no_grad():
            predictions = []
            visualizations = []
            driving_video = x[0]['driving_video']
            source_frame = x[1]['source_video'][:, :, 0, :, :]
